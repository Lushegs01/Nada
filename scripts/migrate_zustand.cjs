...
const filePath = path.join(__dirname, '../apps/web/src/components/screens/Dashboard.tsx');
      ...
      if (expr.getText() === 'useState') {
        ...
        if (nameNode.getKind() === SyntaxKind.ArrayBindingPattern) {
          ...
          if (elements.length >= 2) {
            ...
            if (storeProps.includes(stateName)) {
              // Create the Zustand replacements
              const stateLine = `const ${stateName} = useDashboardStore((state) => state.${stateName});`;
              const setterLine = `const ${setterName} = useDashboardStore((state) => state.${setterName === 'setChatPrefState' ? 'setChatPref' : setterName});`; // Special case handled manually if needed
